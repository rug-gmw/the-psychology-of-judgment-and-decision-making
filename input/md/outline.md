...
**Relationship to confirmation bias:** The representativeness heuristic can also be viewed as a manifestation of confirmation bias. Specifically, in the representativeness heuristic, people do not seek disconfirmatory evidence for how many people matching a propototype of a category nevertheless belong to a different category.
...
**Definition:** Overconfidence manifests in two fundamentally different ways, corresponding to two different distortions of belief. Understanding these two dimensions clarifies an otherwise confusing family of biases. Both can be understood as expressions of [confirmation bias](#confirmation) applied to beliefs about oneself and one's own judgments — but with an additional *motivational* dimension: people are not merely failing to update, they are driven to maintain positive self-regard.
...
**Overprecision (narrowed posterior):** Underestimating the uncertainty in one's own judgments. When asked for 90% confidence intervals, people typically capture the true answer only 40–60% of the time. The most pervasive and least understood form of overconfidence. Here, the posterior is not shifted but *too narrow* — the agent assigns too little probability mass to alternative outcomes. This too reflects confirmation bias: once an estimate is formed, evidence consistent with it is readily accepted while evidence that would widen the distribution is discounted.

**Overconfidence and mental health:** A self-favorable bias appears to be a marker of mental health — which underscores that the motivational component goes beyond cold cognition.
...
- The paradox: a degree of overconfidence — a shifted posterior — appears to be psychologically adaptive, while a more realistic (unshifted) posterior is associated with depression and helplessness. Motivation to confirm a positive self-image is not merely a bias — it is functional.
...
**From expected utility to prospect theory.** Expected value theory (EV = Σ value × probability) is the normative baseline for risky decisions. Expected utility theory (EU) refines this with subjective utilities and captures risk aversion, but is normative rather than descriptive. The Allais Paradox [@allais1953] exposes EU's descriptive failure: people prefer a sure €1,000 over a higher-EV gamble (the **certainty effect**), but reverse their preference when the same utilities appear at lower probabilities. Prospect theory [@kahneman_tversky1979; Kahneman won the 2002 Nobel for this work] is an influential descriptive model that accounts for these departures through two components: a **value function** and a **probability weighting function**.

**The value function** describes how people evaluate outcomes as gains or losses relative to a subjective reference point — typically one's current state or expectations — rather than as final wealth. Many framing effects arise from shifting this reference point. For example, Olympic silver medalists appeared less happy than bronze medalists: silver invited comparison to gold (a loss), bronze to no medal (a gain) [@medvec_etal1995]. The value function has two key properties:

- *Loss aversion:* Losses loom ~2.25× larger than equivalent gains.
- *Diminishing sensitivity:* The function is concave for gains and convex for losses — each additional unit of gain or loss matters less.

A second kind of reference dependence reinforces loss aversion. People evaluate outcomes not only against a fixed reference point, but also relative to *the alternatives*: the average of all possible outcomes serves as an implicit reference. Outcomes below this average produce **regret**; outcomes above it produce **joy**. Because negative emotions are psychologically more powerful than positive ones, this mechanism amplifies the weight of losses — providing a psychological basis for the asymmetry of the value function.
...
A distinct phenomenon occurs at the extremes: very small probabilities (e.g., meteorite impact) are often rounded down to zero, and very high probabilities are treated as certainties. Unlike the overweighting of small-but-noticeable probabilities, this collapsing effect causes people to *ignore* genuine risks entirely — for instance, foregoing insurance because a small probability feels like no probability at all.

**Later extensions to probability judgment.** Subsequent research identified additional distortions beyond the original weighting function:

- *Subadditivity:* People judge P(A ∪ B) as less than P(A) + P(B) — the parts seem to add up to more than the whole. [@tversky_fox1995]
- *Ambiguity aversion:* Known risks are preferred over equivalent unknown risks, even at identical expected values. [@ellsberg1961]
- *Probability neglect:* In emotionally charged scenarios — medical risks, accidents, catastrophes — probabilities may be almost entirely ignored, with judgments driven by the vividness of the outcome alone. [@baron_etal1993]

**Additional biases in risky choice.** Beyond prospect theory itself, several biases shape how people evaluate risky outcomes based on *how* and *why* harm occurs:

- *Nature bias:* Harms from artificial causes are judged worse than equivalent harms from natural causes — e.g., preferences for "natural" over "artificial" foods at equal objective risk. [@kahneman_ritov1994]
- *Omission bias:* Negative consequences from inaction are judged less bad than equivalent consequences from action — e.g., avoiding vaccination even when the disease risk is objectively greater. [@baron_ritov2004]

**Bayesian lens.** A Bayesian agent uses probabilities directly and evaluates outcomes in absolute terms; a prospect-theory agent transforms both. The weighting function w(P) is a biased version of true probability, compounded by subadditivity, ambiguity aversion, and probability neglect. Framing effects mean the same evidence produces different decisions depending on presentation — a direct violation of Bayesian invariance. The reference point determines what counts as a gain or loss, and shifts in this reference point — including toward the average outcome — reshape the entire evaluation.
...
**Definition:** Confirmation bias (or myside bias) is a pervasive tendency to seek, interpret, and favor information that confirms existing beliefs or hypotheses. Proposed as the single cognitive bias most relevant to ideological extremism.
...
**Why positive testing persists — beliefs as action strategies:** People are not only motivated to gather information accurately; they also pursue real-world goals, such as being liked, succeeding at work, or staying safe. When a belief doubles as an action strategy, testing the alternative becomes costly — not just effortful, but *risky*. *Example:* If you believe that always agreeing with others makes them like you, the informative test is to disagree and observe the reaction. But disagreeing risks the very goal (being liked) that the belief serves. This creates an exploration-exploitation tradeoff: exploiting your current belief is safe and goal-consistent; exploring the alternative is epistemically valuable but may jeopardize what you care about. The stronger the goal motivation, the more people default to exploitation — which, from the outside, looks identical to confirmation bias. This is an additional mechanism, layered on top of the cognitive pull toward positive testing.

**2-4-6 task** [@wason1960]: The goal-cost explanation above does not apply to purely abstract hypotheses — yet confirmation bias persists there too, demonstrating that the cognitive default is powerful on its own. In this classic task, participants are told that the sequence 2-4-6 fits a rule, and must discover the rule by proposing new sequences. The actual rule is simply *any three ascending numbers* — but most participants hypothesize a more specific rule, such as "even numbers ascending by 2." They then test confirming sequences (e.g., 8-10-12, 20-22-24) — all of which fit the true rule, reinforcing the wrong hypothesis. Only 21% found the actual rule. Those who succeeded employed the negative test strategy: testing sequences that *violated* their hypothesis, such as 1-3-5 (odd numbers — still ascending, so "yes") or 1-2-3 (ascending by 1 — still "yes") or 6-4-2 (descending — finally "no"). By discovering what *doesn't* work, they narrowed down the real rule.
...
**🎯 Bayesian lens:** A Bayesian agent should weight evidence symmetrically — updating equally from confirming and disconfirming evidence (proportional to their diagnosticity). Confirmation bias produces *asymmetric updating*: confirming evidence strengthens beliefs while disconfirming evidence is discounted or ignored. This creates a self-reinforcing cycle where the posterior drifts ever further from what balanced evidence would warrant. A single disconfirming observation can be enormously informative (high likelihood ratio), yet people treat it as if it has low diagnosticity. The exploration-exploitation tradeoff adds a further wrinkle: even if a Bayesian agent *knows* a negative test would be more diagnostic, the expected cost of a bad outcome may outweigh the expected informational gain — making positive testing *instrumentally rational* despite being *epistemically suboptimal*.
...
**Belief overkill:** When people hold a certain belief, they tend to agree with all evidence in support of this belief, and disagree with all evidence that opposes it. This is even the case for pieces of evidence that are logically unrelated. For example, someone who is generally opposed to AI is likely to feel that AI is harmful to the environment, useless, and harmful to creativity. This person is unlikely to hold a more mixed view such that, for example, AI is harmful to the environment and creativity, yet is useful in many situations.
